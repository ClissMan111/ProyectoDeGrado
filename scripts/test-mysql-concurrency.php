...
if (($argv[1]??'')==='worker') {
    ...
    try {
        if (($argv[6]??'') === 'block') {
            app(IndisponibilidadService::class)->save(['medico_id'=>(int)$argv[4], 'inicio'=>$argv[5].' 08:00:00', 'fin'=>$argv[5].' 09:00:00', 'motivo'=>'QA', 'estado'=>true]);
            echo 'BLOCKED';
            exit;
        }
...
try {
    ...
    for($i=0;$i<2;$i++) {
        ...
        foreach(range(1,7) as $day) Horario::create(['medico_id'=>$m->id,'dia_semana'=>Horario::DIAS[$day],'hora_inicio'=>'08:00','hora_fin'=>'10:00','duracion_cita'=>30,'estado'=>true]);
    }
    foreach(['same_doctor','same_patient','booking_vs_block'] as $scenario) {
        $date=today()->addDays($scenario==='same_doctor'?1:($scenario==='same_patient'?2:3))->format('Y-m-d');
        DB::beginTransaction();
        if($scenario!=='same_patient') Medico::lockForUpdate()->find($doctors[0]->id);
        else Paciente::lockForUpdate()->find($patients[0]->paciente->id);
        $workers=[];
        for($i=0;$i<2;$i++) {
            $worker=new Process([PHP_BINARY,__FILE__,'worker',$name,(string)$patients[$scenario==='same_patient'?0:$i]->id,(string)$doctors[$scenario==='same_patient'?$i:0]->id,$date,($scenario==='booking_vs_block' && $i===1)?'block':'reserve'],base_path());
        ...
        sort($outputs);
        if($scenario==='booking_vs_block') {
            $booked=Cita::whereDate('fecha',$date)->count();
            $blocked=IndisponibilidadMedico::whereDate('inicio',$date)->count();
            if(!in_array($outputs,[['BLOCKED','REJECTED'],['REJECTED','RESERVED']],true) || $booked+$blocked!==1) throw new RuntimeException('Booking/block race failed');
            echo 'PASS booking_vs_block: only one conflicting operation committed'.PHP_EOL;
            continue;
        }
