...
@if(auth()->user()->rol==='administrador')<label>Especialidad<select name="especialidad_id"><option value="">Todas</option>@foreach($especialidades as $e)<option value="{{ $e->id }}" @selected(request('especialidad_id')==$e->id)>{{ $e->nombre }}</option>@endforeach</select></label>@if(request('paciente_id'))<input type="hidden" name="paciente_id" value="{{ request('paciente_id') }}">@endif @endif
