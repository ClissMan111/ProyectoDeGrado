...
<label><span>Fecha</span><input type="date" name="fecha" data-date required min="{{ today()->format('Y-m-d') }}" max="{{ today()->addDays(config('citas.horizonte_dias'))->format('Y-m-d') }}" value="{{ old('fecha',request('fecha',$cita?->fecha->format('Y-m-d')??today()->format('Y-m-d'))) }}"></label></div>
...
<aside class="reservation-aside"><span class="section-tag light">TE ACOMPAÑAMOS</span><h2>Un horario que encaje contigo.</h2><p>Tu reserva se registra como pendiente. Podrás consultar la confirmación en tu panel.</p><ul><li>Trae tu documento de identidad.</li><li>Revisa los datos antes de confirmar.</li><li>@if(config('citas.cambios_minutos')>0)Puedes cancelar o reprogramar hasta {{ config('citas.cambios_minutos') }} minutos antes del inicio.@else Puedes cancelar o reprogramar antes del inicio de la cita.@endif</li></ul><a href="{{ route('how-it-works') }}">Cómo reservar ↗</a></aside></div>
@endsection
